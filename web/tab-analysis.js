...
export function renderAnalysisTab() {
  const d   = state.data;
  const out = d.interview_analysis_data?.output || d.interview_analysis || {};
  ...
  let html = '<div class="ia-tab">';

  // ── Overall + Structural Integrity (11.1) ─────────────────────────────────
  const overallContent = `
    <div class="ia-section-header">Overall Hiring Recommendation</div>
  ...
  html += annotatedSection(overallContent, 'IA_STRUCTURAL', 'interview_analysis.structural_integrity');

  // ── Performance Summary (full width) ─────────────────────────────────────
  if (summary) {
    html += `
      <div class="ia-full-section">
        <div class="ia-section-header">Performance Summary</div>
        <p class="ia-summary-text">${esc(summary)}</p>
      </div>`;
  }

  // ── Role Suitability (full width — no single rubric maps 1:1 here) ────────
  if (suitability.length) {
    const ratingOrder = { 'Excellent': 0, 'Strong': 0, 'Good': 1, 'Proficient': 1, 'Adequate': 2, 'Developing': 3, 'Inadequate': 3, 'Poor': 4, 'Limited': 4 };
    html += `
      <div class="ia-full-section">
        <div class="ia-section-header">Role Suitability</div>
  ...
  // ── Skill Assessment + Score Calibration (11.2) ───────────────────────────
  if (skills.length) {
    const skillsContent = `
      <div class="ia-section-header">Skill Assessment</div>
    ...
    html += annotatedSection(skillsContent, 'IA_CALIBRATION', 'interview_analysis.score_calibration');
  }

  // ── Discussion Points + Discussion Quality (11.3) ─────────────────────────
  if (discussion.length) {
    const dpContent = `
      <div class="ia-section-header">Discussion Points for Next Round</div>
    ...
    html += annotatedSection(dpContent, 'IA_DISCUSSION', 'interview_analysis.discussion_quality');
  }
